...
<title>agent-harness report · ${esc(labels.join(", "))}</title>
<style>${CSS}</style>
</head>
<body>
<main>
...
${runs.map(renderAgentSection).join("\n")}
  ...
  <footer>generated by agent-harness ${esc(opts.version)} · ${esc(opts.generatedAt.toISOString())}</footer>
</main>
<script>${JS}</script>
</body>
</html>
...
/** Best-effort package version for the footer; "0.2.1" when unreadable. */
